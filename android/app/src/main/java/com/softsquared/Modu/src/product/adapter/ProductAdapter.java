package com.softsquared.Modu.src.product.adapter;
...
import com.softsquared.Modu.src.HangulUtils;
...
//            Log.d("로그", "adapter: " + productItem.getmImageUrl());
            ...
            Glide.with(mContext).load(imageUrl).thumbnail(0.5f).placeholder(R.drawable.ic_default).override(50, 50).into(imageView);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return convertView;
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (constraint != null) {
                    for (ProductItem department : mProductAll) {
                        String iniName = HangulUtils.getHangulInitialSound(department.getmBrand());
                        if(iniName.contains(constraint.toString())){
